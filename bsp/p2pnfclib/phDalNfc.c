...
int TAMABaudrateChange      (phHal_sHwReference_t *psHwReference,  uint8_t* can_communicate);
...
//                
					TAMABaudrateChange(psHwReference,&can_communicate);
...
//                    bResult =   Serial232Read       (   psHwReference, Handle, pBuffer, nNumberOfBytesToRead, &NumberOfBytesRead, T_RS232_RETRY, nTimeout, 1);
					   Serial232Read       (   psHwReference->pBoardDriver,  pBuffer, nNumberOfBytesToRead, &NumberOfBytesRead, nTimeout);
                }
                else
                {
...
BOOL SendSerialCommand(phHal_sHwReference_t *psHwReference,  uint8_t *FrameInOut, uint16_t uiBufferInSize, uint8_t *can_communicate)
{
    uint8_t     FrameAck[]      =   {0x00, 0x00,0xFF,0x00,0xFF,0x00};
    uint8_t     FrameError[]    =   {0x00,0x00,0xFF,0x01,0xFF,0x7F,0x81,0x00};
    DWORD       dwDone;
    ...
    *can_communicate =   0;
	...
	phDalNfc_Write( psHwReference,  &FrameInOut[0],uiBufferInSize, 0);

	Serial232Read       (   dev,  &FrameInOut[0], sizeof(FrameAck), &dwDone, 150);
	...
	Serial232Read       (   dev,  &FrameInOut[0], sizeof(FrameError), &dwDone, 150);
    ...
    *can_communicate =   1;
	...
	Serial232Read       (   dev,   FrameInOut+sizeof(FrameError), FrameInOut[3]-1, &dwDone, 1500);
...
//    }
	...
	phDalNfc_Write( psHwReference,  &FrameWakeup[0], sizeof(FrameWakeup), 0);
	
	
	Serial232Read       (   dev,FrameWakeup, sizeof(FrameWakeup), &dwDone, 50);
	Serial232Read       (   dev,FrameWakeup, sizeof(FrameWakeup), &dwDone, 50);
    ...
    if(!SendSerialCommand( psHwReference,&FrameInOut[0], (uint16_t)(uiBufferInSize + 9), can_communicate))
    {
        return FALSE;
    }
...
//    }
	...
	phDalNfc_Write( psHwReference,  &FrameAck[0], sizeof(FrameAck), 0);
