...
#define DATABUFFERSIZE      (uint16_t)  400
...
void SelectTargetPollingMode(phcsExampleTargetParam_t* IParamts)
{
    // set all modes to be able to connect to them
    IParamts->PollModes[0] = phHal_eOpModesNfcPassive106;
    IParamts->PollModes[1] = phHal_eOpModesNfcPassive212;
    IParamts->PollModes[2] = phHal_eOpModesNfcPassive424;
    IParamts->PollModes[3] = phHal_eOpModesNfcActive106;
    IParamts->PollModes[4] = phHal_eOpModesNfcActive212;
    IParamts->PollModes[5] = phHal_eOpModesNfcActive424;
    IParamts->PollModes[6] = phHal_eOpModesArrayTerminator;
...
int nfcTest(sky_comDriver *dev)
{
	...
	while (1)
    {
        ...
        for (int j = 0; j < InitParams.MTU; j++)
        {
            IParam.buffer[j] = j%256;
        }

        IParam.SendLength =  InitParams.MTU; //10;
        ...
        IParam.status = phHalNfc_Transceive(IParam.HwRef,
            ...
            IParam.buffer,
            IParam.SendLength,
            IParam.receivebuffer,
            &(IParam.RcvLength));

       if (IParam.status == NFCSTATUS_SUCCESS)
        {
            rt_kprintf("> 'end' OK\n");
            for (int j = 0; j < IParam.RcvLength; j++)
            {
                if ((j > 0) && ((j % 16) == 0)) { rt_kprintf("\n"); }
                rt_kprintf("%02X ", IParam.receivebuffer[j]);
            }
        }
        else
        {
            rt_kprintf(">recstatus :%d, 'end' error !\n", IParam.status);
			break;
        }
        //Sleep(500);
        //t = timeGetTime() - t;
